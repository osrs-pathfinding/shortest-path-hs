...
nix-shell --run "cabal run route-bench -- --corpus-dir '$corpus_dir' --tier full --runs 3 --oracle '$oracle' --output '$output'"

nix-shell --run \
  "cabal run bench-import -- --corpus-dir '$corpus_dir' --run-id '$run' --testbed '$testbed' --notes '$notes' --clickhouse-url '$clickhouse_url' '$output'"
